...
| Deploy to minikube | `make pipeline && make k8s-deploy` |
| Remove the deployment | `make k8s-delete` |
...
without a model and says so, rather than crash-looping in Kubernetes where a restart
cannot produce a model file.
...
In Kubernetes, `kubectl -n mlops rollout restart deployment/catsdogs-api`.

### The dashboard shows "API not reachable"

Check `ui.api_url`. Inside Compose or Kubernetes it must be the **service name**, not
`127.0.0.1` — each container has its own loopback, so localhost points the dashboard
at itself.

```bash
kubectl -n mlops get configmap catsdogs-config -o yaml | grep API_URL
```

### The log panel is empty in the cluster

Check the Role is applied and bound:

```bash
kubectl -n mlops auth can-i get pods/log \
  --as=system:serviceaccount:mlops:catsdogs-ui
```

If that prints `no`, apply `k8s/02-rbac.yaml`. The panel's message will also say
which collector it tried and why it fell back.
...
kubectl -n mlops rollout status deployment/catsdogs-api
make k8s-smoke
make perf-check API_URL=$(minikube service catsdogs-api -n mlops --url)
```
...
A failure on `request_success` alone is an infrastructure issue; look at pod
